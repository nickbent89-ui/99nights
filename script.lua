...
Tabs.Main:Section({ Title = "Automated Sustenance", Icon = "utensils" })
            ...
            task.spawn(function()
                while autoFeedToggle do
                    ...
                    if wiki(selectedFood) == 0 then
                        ...
                        Tabs.Main:Find("Enable Auto-Feed"):SetValue(false)
...
Tabs.Auto:Toggle({
    Title = "Activate Campfire Upgrade",
    Value = false,
...
Tabs.Tp:Button({ -- Renamed for consistency
    Title = "Warp to Stronghold",
    Locked = false,
    Callback = function()
        tp2()
    end
...
local MobDropdown = Tabs.Tp:Dropdown({
    Title = "Select Child", -- Renamed for consistency
...
local ChestDropdown = Tabs.Tp:Dropdown({
    Title = "Select Chest", -- Renamed for consistency
...
local fuelItems = {"Log", "Chair", "Coal", "Fuel Canister", "Oil Barrel"}
local selectedFuelItems = {} -- Already defined, but keeping for context

Tabs.br:Dropdown({
    Title = "Choose Energy Items",
    Desc = "Choose items to bring",
    Values = fuelItems,
    Multi = true,
    AllowNone = true,
...
local foodItems = {"Cake", "Cooked Steak", "Cooked Morsel", "Steak", "Morsel", "Berry", "Carrot"}
local selectedFoodItems = {} -- Already defined, but keeping for context

Tabs.br:Dropdown({
    Title = "Choose Provisions",
    Desc = "Choose items to bring",
    Values = foodItems,
    Multi = true,
    AllowNone = true,
...
Tabs.br:Button({
    ...
    Callback = function()
        ...
        if not player.Character or not player.Character:FindFirstChild("HumanoidRootPart") then return end
        local hrp = player.Character.HumanoidRootPart
        local targetPos = hrp.Position + Vector3.new(2, 0, 0) -- Adjust as needed
...
local equipmentItems = {"Revolver", "Rifle", "Leather Body", "Iron Body", "Revolver Ammo", "Rifle Ammo", "Giant Sack", "Good Sack", "Strong Axe", "Good Axe"}
local selectedEquipmentItems = {} -- Already defined, but keeping for context
...
function createESPText(part, text, color)
    ...
    label.Text = text
    label.TextColor3 = Color3.fromRGB(255, 0, 0) -- Changed to red
    label.TextStrokeTransparency = 0 -- Removed stroke transparency for better texture
    label.TextScaled = true
    label.Font = Enum.Font.SourceSansBold -- Changed font to SourceSansBold for better texture
...
local function Aesp(nome, tipo)
    ...
    for _, obj in ipairs(container:GetChildren()) do
        if obj.Name == nome then
            ...
            if part then
                createESPText(part, obj.Name, Color3.fromRGB(255, 0, 0)) -- Always red
            end
        end
    end
end
...
Tabs.Info:Button({
    ...
    Callback = function()
        local success, err = pcall(function()
            ...
            setclipboard("https://discord.gg/yRMdx5KXGq")
        end)
        if not success then
